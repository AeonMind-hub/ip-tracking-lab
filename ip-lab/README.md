...
quickstart (60 seconds, no privileges)
  cd ip-lab
  python3 tools/selftest.py            # 166 checks: prove the lab works, then trust it
...
python3 tools/selftest.py     # 166 checks: parsers, trust walk, pcap round-trip, rules, configs, shop guards
...
├── tools/
│   ├── sync.py                commit + write lab.bundle + verify (+ --push to a remote)
│   ├── selftest.py            166 checks; run it after every edit
...
│                              + test_cert.pem (a public-key-only fixture for parse_pem)
