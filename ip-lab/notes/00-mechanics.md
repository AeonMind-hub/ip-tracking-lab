...
```bash
python3 tools/selftest.py          # 165 checks: the lab tests its own claims
...
1. **Every claim is a test.** If the README says a config produces a finding, there's an
   assertion. Docs rot; `RESULT: 165 checks passed` doesn't. A check that cannot run because the
